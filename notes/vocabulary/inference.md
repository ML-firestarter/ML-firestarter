...
Training happens once, but inference runs for every request from every user, so for a popular model the total cost of inference can exceed the cost of training. Making inference cheaper and faster, with smaller models, quantization, batching and better hardware, is a large part of ML engineering.
...
**Related:** [Token](token.md) · [Pretraining](pretraining.md) · [Revenue model](revenue-model.md)
