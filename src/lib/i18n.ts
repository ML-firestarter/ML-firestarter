...
const en = {
  ...
  // Exercises, whose code runs in the reader's browser, and the lessons' examples that readers can run.
  exercises: {
    title: 'Exercises',
    exerciseOf: (i: number, n: number) => `Exercise ${i} of ${n}`,
    pageTitle: (title: string) => `Exercise: ${title}`,
    // On the lesson's page, above its exercises.
    lede: 'Practise what this lesson covers. Your code runs in your browser, where the site checks it and keeps it for you.',
    progress: '{} exercises passed',
    passed: 'Passed',
    editor: 'Your code',
    input: 'Input',
    inputHint: 'What your code reads with input(), one line each.',
    run: 'Run',
    check: 'Check',
    stop: 'Stop',
    reset: 'Start over',
    resetHint: 'Puts back the code you started with. Ctrl+Z in the editor brings yours back.',
    shortcut: '{} runs your code.',
    output: 'Output',
    results: 'Checks',
    solution: 'Our solution',
    solutionNote: "Yours doesn't have to look the same: any code that passes the checks is right.",
    pager: 'Other exercises',
    // The lessons' examples.
    edit: 'Edit',
    restore: 'Undo changes',
    // Plain text and plural forms only: pages pass them to exercise.ts and runnable.ts as JSON, which fill in `{…}`.
    script: {
      loading: 'Loading Python… The first time takes a few seconds.',
      checking: 'Checking…',
      nothing: 'Your program ran, and printed nothing.',
      stopped: 'Stopped.',
      ranOut: 'Your program wanted another line of input, but the Input box has no more. Type what it should read there, one line at a time.',
      tooMuch: 'Your program printed so much that it was stopped. Is there a loop that never ends?',
      unavailable: "Python couldn't be loaded. Check your connection, then try again.",
      crashed: 'Python stopped working ({message}). Try again.',
      // Checking an exercise.
      summary: { one: '{passed} of # check passed.', other: '{passed} of # checks passed.' } as PluralForms,
      allPassed: 'All checks passed! Well done.',
      compare: 'Compare your code with ours below.',
      codeError: "Your code has an error, so the checks couldn't run:",
      codeTooMuch: 'Your code printed so much that it was stopped before the checks could run. Is there a loop that never ends?',
      broken: "This exercise's checks have a mistake, so they can't run:",
      timedOut: 'Check {number} took longer than {seconds} seconds, so checking stopped. Is there a loop that never ends?',
      // What a check tried, before what happened.
      program: 'Your program',
      programWith: 'Your program, with the input {input}',
      returns: 'Returns {expected}.',
      raisesRight: 'Raises {expected}, as it should.',
      printsRight: 'Prints the right text.',
      returnsInstead: 'Returns {got} instead of {expected}.',
      raisesInstead: 'Raises an error instead of returning {expected}:',
      returnsNotRaises: 'Returns {got} instead of raising {expected}.',
      raisesOther: 'Raises a different error instead of {expected}:',
      printsInstead: 'Prints {got} instead of {expected} on line {line}.',
      printsTooFew: 'Stops printing before line {line}, which should be {expected}.',
      printsTooMany: 'Prints {got} on line {line}, where the output should end.',
      stopsWithError: 'Stops with an error:',
      printsTooMuch: 'Prints so much that it was stopped. Is there a loop that never ends?',
      printedInstead: 'It printed this, though. Did you use print() where you meant return?',
...
const pl: typeof en = {
  ...
  exercises: {
    title: 'Ćwiczenia',
    exerciseOf: (i, n) => `Ćwiczenie ${i} z ${n}`,
    pageTitle: (title) => `Ćwiczenie: ${title}`,
    lede: 'Przećwicz to, czego uczy ta lekcja. Kod działa w Twojej przeglądarce: tam strona go sprawdza i przechowuje.',
    progress: 'Zaliczone ćwiczenia: {}',
    passed: 'Zaliczone',
    editor: 'Twój kod',
    input: 'Wejście',
    inputHint: 'To, co kod wczytuje przez input(), każda wartość w osobnym wierszu.',
    run: 'Uruchom',
    check: 'Sprawdź',
    stop: 'Zatrzymaj',
    reset: 'Zacznij od nowa',
    resetHint: 'Przywraca kod, od którego zaczynasz. Ctrl+Z w edytorze przywraca Twój.',
    shortcut: '{} uruchamia kod.',
    output: 'Wynik',
    results: 'Sprawdzenie',
    solution: 'Nasze rozwiązanie',
    solutionNote: 'Twoje nie musi wyglądać tak samo: dobry jest każdy kod, który przechodzi sprawdzenia.',
    pager: 'Inne ćwiczenia',
    edit: 'Edytuj',
    restore: 'Cofnij zmiany',
    script: {
      loading: 'Wczytywanie Pythona… Za pierwszym razem trwa to kilka sekund.',
      checking: 'Sprawdzanie…',
      nothing: 'Program zadziałał i nic nie wypisał.',
      stopped: 'Zatrzymano.',
      ranOut: 'Program chciał wczytać kolejny wiersz, ale w polu Wejście nie ma już więcej. Wpisz tam, co ma wczytać, każdą wartość w osobnym wierszu.',
      tooMuch: 'Program wypisał tak dużo, że został zatrzymany. Czy jakaś pętla nigdy się nie kończy?',
      unavailable: 'Nie udało się wczytać Pythona. Sprawdź połączenie z internetem i spróbuj ponownie.',
      crashed: 'Python przestał działać ({message}). Spróbuj ponownie.',
      summary: { other: 'Zaliczone sprawdzenia: {passed} z #.' },
      allPassed: 'Wszystkie sprawdzenia zaliczone! Dobra robota.',
      compare: 'Porównaj swój kod z naszym poniżej.',
      codeError: 'W kodzie jest błąd, więc sprawdzenia nie mogły się uruchomić:',
      codeTooMuch: 'Kod wypisał tak dużo, że został zatrzymany, zanim sprawdzenia mogły się uruchomić. Czy jakaś pętla nigdy się nie kończy?',
      broken: 'Sprawdzenia tego ćwiczenia mają błąd, więc nie mogą się uruchomić:',
      timedOut: 'Sprawdzenie {number} trwało dłużej niż {seconds} s, więc sprawdzanie przerwano. Czy jakaś pętla nigdy się nie kończy?',
      program: 'Twój program',
      programWith: 'Twój program z wejściem {input}',
      returns: 'Zwraca {expected}.',
      raisesRight: 'Zgłasza {expected}, zgodnie z oczekiwaniem.',
      printsRight: 'Wypisuje właściwy tekst.',
      returnsInstead: 'Zwraca {got} zamiast {expected}.',
      raisesInstead: 'Zgłasza błąd zamiast zwrócić {expected}:',
      returnsNotRaises: 'Zwraca {got} zamiast zgłosić {expected}.',
      raisesOther: 'Zgłasza inny błąd zamiast {expected}:',
      printsInstead: 'Wypisuje {got} zamiast {expected} w wierszu {line}.',
      printsTooFew: 'Kończy wypisywanie przed wierszem {line}, który powinien brzmieć {expected}.',
      printsTooMany: 'Wypisuje {got} w wierszu {line}, a wynik powinien się już skończyć.',
      stopsWithError: 'Kończy się błędem:',
      printsTooMuch: 'Wypisuje tak dużo, że sprawdzanie przerwano. Czy jakaś pętla nigdy się nie kończy?',
      printedInstead: 'Za to coś wypisuje. Czy print() jest tam, gdzie powinno być return?',
      emptyLine: '(pusty wiersz)',
      expectedOutput: 'Oczekiwany wynik',
